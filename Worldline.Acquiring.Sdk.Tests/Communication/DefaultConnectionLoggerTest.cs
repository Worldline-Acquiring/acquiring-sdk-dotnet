...
namespace Worldline.Acquiring.Sdk.Communication
{
  ...
  headers:      'X-WL-ServerMetaInfo=""") + @"[^""]*" + Regex.Escape(@""", Date=""") + @"[^""]*" + Regex.Escape(@""", Trace-Id=""") + @"[^""]+" + Regex.Escape(@""", Authorization=""********""") + "[^']*" + Regex.Escape("'");
  ...
  method:       'POST'
  uri:          '/v1/created'
  headers:      'X-WL-ServerMetaInfo=""") + @"[^""]*" + Regex.Escape(@""", Date=""") + @"[^""]*" + Regex.Escape(@""", Trace-Id=""") + @"[^""]+" + Regex.Escape(@""", Authorization=""********"", Content-Type=""application/json""") + "[^']*" + Regex.Escape(@"'
  content-type: 'application/json'
  body:         '{""card"":{""cardSecurityCode"":""***"",""cardNumber"":""************3456"",""expiryDate"":""**2024""}}'");
  ...
  body:         'Not Found'");
        ...
        public async Task TestDeleteWithVoidResponse()
        {
            ...
            using (var _ = new MockServer(5357, "/v1/void", (request, response, arg3) =>
                   {
